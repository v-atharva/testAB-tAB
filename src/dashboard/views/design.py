...
def render() -> None:
    ...
    theme.hero(
        ...
        f"about <b>{ui.pct(typical_rel)} relative</b> (one sd of the fitted prior "
        f"on true lifts), around a mean CTR of <b>{ctr_mean:.1%}</b>. Plan for "
    ...
    theme.section("How many impressions per arm do I need?")
    rel = st.slider(
    ...
    plan_cards = [{
        "label": f"impressions per arm · {power:.0%} power vs {rel:.0%} lift",
        "value": f"{n_needed:,}",
        "tone": "grad",
    }]
    if ratio > 1:
        plan_cards.append({
            "label": "vs the median archived arm",
            "value": f"{ratio:.1f}x",
            "sub": f"the median Upworthy arm had ~{median_arm:,} impressions — "
                   "this gap is why most of the archive is underpowered",
        })
    theme.stat_grid(plan_cards)

    theme.section("What could a test of my size actually detect?")
    ...
    theme.stat_grid([
        {
            "label": "minimum detectable lift · relative",
            "value": ui.pct(mde_rel, signed=False),
            "sub": f"smallest upward lift detectable at {power:.0%} power: "
                   f"{mde_abs * 100:.2f} pp absolute on a {p0:.1%} baseline",
        },
        {
            "label": "power vs a corpus-typical lift",
            "value": f"{achieved:.0%}",
            "dot": "ok" if achieved >= 0.5 else "warn",
            "sub": f"chance of detecting a {ui.pct(typical_rel)} relative lift "
                   "if one were truly there",
        },
    ])
    if achieved < 0.5:
        st.warning(
